...
| 資料新鮮度 | 開放平台原始發布頻率 | 每週日 02:00 自動 ETL，可後台手動觸發 |
...
| 資料類別 | 取得方式 | 來源 | 更新方式 |
|---|---|---|---|
| **路線、站點、班表（三市）** | 開放資料 + 排程爬蟲 | data.taipei、data.ntpc.gov.tw、opendata-kl.askeycloud.com | 每週日 02:00 自動 ETL，亦可後台手動觸發 |
...
| **APScheduler** | 3.10.4 | 背景排程（每 60 秒推播、每週日 02:00 ETL）|
